// 纯JS省市区三级联动
var addressInit = function(_cmbProvince, _cmbCity, _cmbArea,_initArr,arr)
{
    ...
    function changeCity()
    {
        ...
        cmbSelect(cmbArea,_initArr?_initArr[2]:null);
    }
    function changeProvince()
    {
        ...
        cmbSelect(cmbCity,_initArr?_initArr[1]:null);
        changeCity();
    ...
    if(arr){
        provinceList.map(function (item) {
            var _item=item;
            if(contains(arr,item.name)){
                var _arr=contains(arr,item.name);
                for(var k in _arr){
                    var _len=_arr[k].length;
                    if(_len>0){
                        var city=item.cityList.filter(function (item) {
                            for(var i=0;i<_len;i++){
                                if(_arr[k][i]===item.name){
                                    return item;
                                }
                            }
                        });
    ...
    {
        cmbAddOption(cmbProvince, prov[i].name, prov[i]);
    }
    cmbSelect(cmbProvince,_initArr?_initArr[0]:null);
    changeProvince();
