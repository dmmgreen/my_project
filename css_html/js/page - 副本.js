    ...
    $.fn.textify = function (t) {
        ...
        return this.each(function () {
            ...
            function setImgWidth(e, t) {
                if (e.width > s) {
                    t.css({display: "block", width: s, margin: 0, padding: 0});
                    thisNewHeight = e.height * t.width() / e.width;
                    if (thisNewHeight >= o) {
                        t.css({display: "block", width: "auto", margin: 0, height: o, padding: 0})
                    }
                } else {
                    if (e.height >= o - 100) {
                        t.css({display: "block", height: o - 100, margin: 0, padding: 0})
                    } else {
                        t.css({display: "block", "margin-right": "20px", "margin-top": "10px", "margin-bottom": "10px"})
                    }
                }
            }
                    ...
                    opt.contents().each(function () {
                        ...
                        if (f.height() > o) {
                            ...
                            $('[data-over]').each(function () {
                             $(this).height(c-t.padding*2-t.margin*2);
                             console.log($(this).height(),c-t.padding*2-t.margin*2)
                             });
                        }
                    })
                }
            ...
            function x(opt, n) {
                if (opt.contents().length > 0 && v.text().length > 0) {
                    ...
                    opt.contents().each(function (n) {
                        ...
                        if (f.height() > o) {
                            model.detach();
                            /*if (this.nodeName === "IMG") {//超出高度节点为iframe
                             var img=this,
                             u=$this.find('.contentText');
                             img.height=o-a.children(":last").height();
                             t.startPage++;
                             r = $this.children(":first");
                            ...
                            if (this.nodeName === "IFRAME") {//超出高度节点为iframe
                                var iframe=this;
                                iframe.height=o-a.children(":last").height();
                                // $(this).appendTo(theBox);
                            ...
                            //超出高度里的子节点大于0(超出节点为iframe会报错)
                                ...
                                x($(this), theBox);
                                return false
                            } else if ($(this)[0].nodeName === "IMG") {//超出高度节点为img
                                $(this).height(o-f.height());
                                console.log(o,f.height())
                                $(this).attr('data-over',true);
                                pageNav();//添加另一页
                                return false
                            }else {
